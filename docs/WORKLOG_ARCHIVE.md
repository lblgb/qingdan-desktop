...
本文档保留旧版完整工作日志快照，主要用于历史回溯。
当前活跃工作日志请查看 [`docs/WORKLOG.md`](./WORKLOG.md)。
...
- 对 `v0.20` 做最后一次文档自洽修订，只处理当前轮自身的可发现性和口径收束。

### 当前结论

- `v0.20` 仍是唯一当前实施基线；本轮不改范围、不改行为验收口径。
- 本轮增量仅包括：在 `docs/PROJECT_CONSTRAINTS.md` 的阶段/历史节补充 `v0.20` 当前活跃版本与实施基线指针；收敛本轮 `docs/WORKLOG.md` 表述，使其只保留第 37 轮自身增量。

### 决策原因

- 第 36 轮已经完成 `v0.20` 基线建立；继续重复列出同一批文档变化只会降低追溯信噪比。
- 产品验收清单应只保留可执行的版本行为检查，不混入文档同步类流程项。

### 文档更新

- 调整了 `docs/PROJECT_CONSTRAINTS.md`，在阶段/历史节补充 `v0.20` 当前活跃版本与 `docs/ARCHITECTURE.md` + `docs/V020_ACCEPTANCE.md` 的实施基线说明。
- 压缩了 `docs/WORKLOG.md` 第 37 轮内容，只保留本轮文档自洽修订对应的增量表述。

### 实现记录

- 本轮仅处理文档结构与验收口径纯化，没有代码改动。

### 验证记录

- 已复查本轮未改动 `v0.20` 的产品范围、功能边界和行为验收项，只清理可发现性与流程噪音。

### 下一步建议

- 后续进行 `v0.20` 实现时，继续只以前置后的 `v0.20` 基线和纯化后的验收清单为准。
