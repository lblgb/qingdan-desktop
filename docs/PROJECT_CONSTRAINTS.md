...
> `v1.0.0` 继续执行“文档先行”原则。任何新增范围、验收项、实现边界或行为变化，都应先更新架构文档、验收文档或本文档，再进入代码实现。
...
- 所有文本文件默认统一使用 `UTF-8` 无 BOM；适用范围包括源码、测试、脚本、文档、JSON、Markdown、`toml/yaml` 和样式文件，不只限于配置文件。
- 禁止提交出现乱码、问号替代、BOM 污染或错误转码的中文内容；一旦发现，必须先修复编码问题，再继续功能开发或提交。
- 对包含中文的文件进行脚本化改写、批量替换或跨工具链处理后，必须复查 git diff，确认中文显示正常，不能以“功能可运行”代替编码验收。
- 文本配置文件统一使用 UTF-8 无 BOM，尤其是 `Cargo.toml`、`tauri.conf.json` 等会被构建工具直接解析的文件。
- 若仓库存在 `.editorconfig`、格式化器或编辑器工作区设置，编码规则必须与本条保持一致，不得出现局部例外。
