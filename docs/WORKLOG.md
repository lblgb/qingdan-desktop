...
- 对第 36 轮之后的 `v0.20` 文档纯化增量做最后收尾，只处理可发现性、日志压缩和验收口径纯化。

### 当前结论

- `v0.20` 仍是唯一当前实施基线；本轮不改范围、不改行为验收口径。
- 本轮增量仅包括：将 `docs/ARCHITECTURE.md` 中的 `v0.20` 基线前置、压缩本轮日志重复表述、移除 `docs/V020_ACCEPTANCE.md` 中非产品行为的流程项。
...
- 调整了 `docs/ARCHITECTURE.md`，将 `v0.20` 当前实施基线前置到历史归档之前。
- 压缩了 `docs/WORKLOG.md` 第 37 轮内容，只保留相对第 36 轮新增的文档纯化信息。
- 调整了 `docs/V020_ACCEPTANCE.md`，收拢非产品行为的流程性验收项，保留构建验证检查。
