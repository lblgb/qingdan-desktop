...
- 当前验收清单：[`docs/V100_ACCEPTANCE.md`](./V100_ACCEPTANCE.md)
- 历史日志归档：[`docs/WORKLOG_ARCHIVE.md`](./WORKLOG_ARCHIVE.md)

## 版本索引

- 活跃轮次：
  - [`2026-05-07 第 50 轮`](#2026-05-07-第-50-轮)
...
- 近期关键节点：
  ...
  - `v1.0.0` 当前实施基线：见 [`docs/ARCHITECTURE.md`](./ARCHITECTURE.md) 与 [`docs/V100_ACCEPTANCE.md`](./V100_ACCEPTANCE.md)
...
- 将 `v1.0.0` 周期任务从文字进展提交升级为时间驱动的目标分钟完成模型。

### 当前结论

- 周期任务以每周期目标分钟数作为完成标准，默认以分钟为统一时间单位。
- “提交进展”替换为“开始计时”，计时结束或退出后生成时间记录，并用记录累计当前周期完成情况。
- 两段不连续时间记录保持为两条独立记录，不做自动合并，累计分钟数用于达标判断。
- 周期任务不关联普通任务，继续保持独立数据模型和任务清单内切换展示。

### 实现记录

- 后端新增周期任务目标分钟字段与周期时间记录表，补齐创建、列出、修改说明、删除时间记录命令。
- 前端周期任务列表改为时间完成视角，支持计时、暂停、继续、结束、保存记录、详情历史编辑与删除。
- 增加计时会话本地恢复能力，刷新或异常退出后可恢复计时、保存为记录或丢弃。
- 任务概览新增周期任务整体目标、完成、剩余和单项最近记录展示。

### 验证记录

- 已执行 `npm.cmd test -- recurring.storage`。
- 已执行 `npm.cmd test -- RecurringTaskList recurring.storage`。
- 已执行 `npm.cmd test -- TaskList`。
- 已执行 `npm.cmd run build`。

### 下一步建议

- 手工验证计时弹窗、退出生成记录、恢复计时、详情历史记录修改/删除和任务概览统计。
- 手工验收通过后，再按发布流程构建安装包、打 tag 并发布 GitHub Release。
...
- 为 `v0.40.0` 补齐文档基线、验收清单和活跃指针，确保后续实现有唯一可执行口径。

### 当前结论

- `v0.40.0` 的当前实施边界已经从 `v0.30.0` 切换过来，新的唯一验收清单是 `docs/V040_ACCEPTANCE.md`。
- 本轮已按验收清单完成实现闭环，不改动其他版本的历史记录。
- 活跃基线现在覆盖运行期桌面通知、`completed_at`、归档不删除、任务详情备注和复盘概览。

### 文档更新

- 新增 [`docs/V040_ACCEPTANCE.md`](./V040_ACCEPTANCE.md)，作为 `v0.40.0` 当前唯一有效的手工验收清单。
- 更新 [`docs/ARCHITECTURE.md`](./ARCHITECTURE.md)，把活跃版本切到 `v0.40.0`，并补齐当前实施基线说明。
- 更新 [`docs/WORKLOG.md`](./WORKLOG.md)，记录 `v0.40.0` 文档基线收口结果。

### 实现记录

- 完成 `note / completed_at / archived_at` 数据模型和兼容迁移。
- 完成任务详情弹窗、备注编辑、归档视图和批量归档。
- 完成复盘统计、最近完成、月度趋势和质量提示。
- 完成桌面通知权限状态和测试通知入口。
- 根据最终审查修复桌面端归档全量查询、旧完成任务 `completed_at` 回填和批量完成补齐空完成时间。
...
- `cargo check` 通过。
- 未执行交互式 `tauri:dev` 手工烟测；当前终端环境无法可靠完成桌面界面人工验收。
...
- 顶部已增加轻量搜索输入，可按标题与备注搜索任务，并且以全量任务集作为搜索基线。
