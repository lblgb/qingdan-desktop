...
- 当前活跃版本：
  - [`v0.20` 文档基线整理（第 36 轮）](#2026-04-11-第-36-轮)
  - [`v0.20` 文档收尾对齐（第 37 轮）](#2026-04-11-第-37-轮)
- 近期关键节点：
  - [`v0.1.5` 收口确认（第 34 轮）](#2026-04-11-第-34-轮)
  - [`v0.1.5` 发布资产补齐（第 35 轮）](#2026-04-11-第-35-轮)
- 历史轮次：自下方按时间顺序继续查阅。
...
- 将任务组筛选并入左侧“更多条件”的二级层级，同时把任务组管理收回顶部独立入口，并为任务清单补上分页。
### 当前结论

- 左侧侧栏现在只保留一张 `工作视图` 卡片。
- `任务组` 已不再作为独立侧栏卡片存在，而是被并入 `更多条件 -> 任务组 -> 全部组 / 未分组 / 某个组` 这一层级。
- 顶部动作区现在是：
  - `任务概览`
  - `任务组管理`
  - `新建`
- `任务组管理` 已改成独立弹窗入口，负责任务组的新增、编辑和删除，不再占用主界面布局。
- 任务清单现在增加了前端分页：
  - 默认每页 10 项
  - 支持上一页 / 下一页 / 页码切换
  - 切换主筛选或附加筛选时自动回到第 1 页
### 决策原因

- 任务组属于附加属性，更适合作为“更多条件”中的一个二级筛选，而不是与任务主状态并列占据左栏主体。
- 任务组管理本身属于低频结构操作，收为顶部独立动作入口后，主界面会更聚焦任务清单。
...
- 你现在可以重点看三件事：
  - `更多条件 -> 任务组` 这个二级层级是否顺手
  - 顶部 `任务组管理` 弹窗的层级是否比之前更合理
  - 清单分页是否解决了列表过长的问题
- 如果这轮交互结构没问题，后面就可以继续收更细的体验项，而不是再回头改大布局。
## 2026-04-11 第 34 轮
### 讨论主题

- 结合当前完成状态，正式确认 `v0.1.5` 收口，并整理版本基线文档。
### 当前结论

- `v0.1.5` 已确认收口。
- 当前第二版的范围已完成并稳定，后续不再继续向该版本塞入新的结构性功能。
- 已补充独立收口文档，明确当前完成项、非范围项和下一阶段建议方向。
### 决策原因

- 当前版本已经完成“任务组织能力升级”和“主界面交互收束”的核心目标。
- 如果继续在 `v0.1.5` 内追加新功能，会重新模糊版本边界，也不利于后续规划下一阶段。
...
- 当前工作区除本地 Python 安装器外已处于可提交状态。
- `v0.1.5` 的代码基线此前已通过 `tsc -b` 与 `cargo check`。
### 下一步建议
...
- 正式进入 `v0.20` 的实施规划与文档基线整理，先把优先级、更多条件、批量操作、快捷操作强化、系统级界面精修和概览增强的边界写清楚。

### 当前结论

- `v0.20` 采用“文档先行、实现后置”的推进方式，先统一架构与验收基线，再开始功能代码。
- 本轮优先级正式落地、更多条件体系扩展、轻量批量操作、快捷操作强化、系统级界面精修和任务概览增强都需要共享同一套语义基线。
- 当前阶段先不扩展到快速新建条、批量删除、日历主视图和服务端同步，避免范围再次膨胀。

### 决策原因

- `v0.20` 已经进入稳健扩展阶段，若不先把边界写进文档，后续很容易在筛选、排序、批量模式、快捷动作和概览之间重新交叉耦合。
- 统一验收清单能把“做到什么算完成”说清楚，减少后续回头补口径的成本。
- 先收口文档再推进实现，可以让后续每一步改动都对齐到同一个版本基线。
...
- 按 `docs/ARCHITECTURE.md` 和 `docs/V020_ACCEPTANCE.md` 进入后续实现。
## 2026-04-11 第 37 轮
### 讨论主题

- 完成 `v0.20` 文档债收尾，专门清理“当前/历史”表述冲突、归档层级混乱和工作日志重复记录。

### 当前结论

- 本轮只做文档对齐增量，不重写第 36 轮已经确定的 `v0.20` 范围与验收基线。
- `docs/PROJECT_CONSTRAINTS.md` 已明确当前阶段只以 `v0.20` 为准，`v0.1.5` 改为历史参考。
- `docs/ARCHITECTURE.md` 已把 `v0.1.5` 与更早范围整理进清晰的历史归档层级，不再挂在容易误读为活跃基线的标题之下。
- 本工作日志第 37 轮仅保留上述收尾价值，不再重复第 36 轮的基线整理内容。

### 决策原因

- 第 36 轮已经完成 `v0.20` 基线建立，本轮只需要处理扫描性和表述一致性问题，避免连续轮次重复记同一件事。
- “当前实施基线”和“历史归档”混写会提高后续按文档执行时的误判风险。
- 工作日志应区分“建立基线”和“清理文档债”两类工作，保证每轮的增量价值可追溯。

### 文档更新

- 调整了 `docs/PROJECT_CONSTRAINTS.md`，移除将 `v0.1.5` 写成当前阶段重点/边界的残留表述。
- 调整了 `docs/ARCHITECTURE.md`，重排历史版本归档的标题层级与结构说明。
- 压缩了 `docs/WORKLOG.md` 第 37 轮内容，只保留文档对齐增量。
...
- 已复查第 36 轮与第 37 轮分工：前者负责建立 `v0.20` 基线，后者负责收尾校对与文档债压缩。
- 本轮未引入新的产品范围、验收项或实施边界。

### 下一步建议

- 后续进行 `v0.20` 实现时，继续只以当前文档中的 `v0.20` 基线为准；历史章节仅作回溯参考。
